...
router
  .route("/")
  .post(
    validate(ZCreateQuiz),
    authenticateToken,
    asyncHandler(QuizController.createQuiz)
  );

router.route("/").get(asyncHandler(QuizController.getAllQuiz));
...
router
  .route("/question/:questionId")
  .patch(
    validate(ZQuestionUpdate),
    asyncHandler(QuizController.updateQuestion)
  );

// get quiz with questions
router.route("/:quizId").get(asyncHandler(QuizController.getCompleteQuiz));

router.route("/:quizId").delete(asyncHandler(QuizController.deleteQuiz));
