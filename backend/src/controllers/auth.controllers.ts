...
export class AuthController {
  ...
  static async verifyOtp(req: Request, res: Response) {
    ...
    if (role === "STUDENT") {
      ...
      if (!studentData) {
        throw new ApiError(
          "No student exist with phone number please register first.",
      ...
      if (!adminData) {
        throw new ApiError(
          "No admin exist with phone number.",
          HTTP_STATUS.BAD_REQUEST
        );
      }
      userId = adminData.id;
    ...
    otpHandler.deleteOtp(phoneNumber);
    if (userId == null) {
      // Defensive: should not happen because we validated above, but keep safe for TS
      throw new ApiError("User ID missing", HTTP_STATUS.INTERNAL_SERVER_ERROR);
    }
  ...
  static async getCurrentUser(req: Request, res: Response) {
    ...
    // Accept either `user.userId` or `user.id`. Only reject when null/undefined
    const userId = (user.userId ?? user.id) as string | number | undefined;
    if (userId == null) {
      logger.info("getCurrentUser: req.user shape:", { user });
      throw new ApiError("User ID required", HTTP_STATUS.UNAUTHORIZED);
    }

    let userData;
    let studentTeam;
    if (user.role === "STUDENT") {
      userData = await StudentRepository.getById(String(userId));
      studentTeam = await StudentRepository.getTeamByStudentId(
        userData?.teamId
      );
    } else {
      userData = await AdminRepository.getById(String(userId));
    }

    if (!userData) {
      throw new ApiError("No user found", HTTP_STATUS.NOT_FOUND);
    }

    ApiResponse.success(
      res,
      { userData, team: studentTeam, role: user.role },
