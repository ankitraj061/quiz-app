...
export const validate = (schema: ZodSchema): RequestHandler => {
  return (req, res, next): void => {
    ...
    if (!result.success) {
      ...
      const validationErrors: ValidationErrorDetail[] = errors.map((err: { path: any[]; code: string; message: string;  expected: any; }) => ({
        ...
        // Only include value in non-production for debugging
      ...
      return next(new ApiError(mainMessage, 400, validationErrors));
    }

    // Validation passed - use parsed and validated data
